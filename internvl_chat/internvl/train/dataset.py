...
def draw_click_to_image(image, x, y, transparency=0.5):
    """
    Draw a transparent circle at the specified position on the image.

    Parameters:
        image (PIL.Image.Image): The original image in RGB format.
        x (float): The x-coordinate of the click (normalized to 1000).
        y (float): The y-coordinate of the click (normalized to 1000).
        transparency (float): The transparency level of the circle (0.0 to 1.0).

    Returns:
        PIL.Image.Image: The modified image with the transparent circle.
    """
...
def draw_click_to_image_pretrain(image, x, y, transparency=0.5):
    """
    Draw a transparent circle at the specified position on the image.
...
def draw_bbox_to_image(image, bbox, color=(255, 0, 0)):
    """
    Draws a bounding box 3 pixels outside the specified box while respecting image boundaries.
    
    :param image: PIL Image object
    :param bbox: List of four integers [x1, y1, x2, y2] in a normalized format (0 to 1000)
    """
    x1, y1, x2, y2 = bbox
    width = image.width
    height = image.height
