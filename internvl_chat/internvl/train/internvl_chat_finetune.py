...
from internvl.train.dataset import (ConcatDataset, TCSLoader,
                                    ...
                                    dynamic_preprocess, dynamic_preprocess_v3, preprocess,
                                    preprocess_internlm,
                                    preprocess_internvl2_5, preprocess_internvl_v3,
                                    preprocess_mpt, preprocess_phi3, draw_click_to_image, draw_click_to_image_pretrain, draw_bbox_to_image)
...
# Try to import aoss_client for image loading, fallback to PIL if unavailable
try:
    from aoss_client.client import Client
    has_tcs_loader = True
except ImportError as E:
    print('aoss_client is not installed. Using PIL to load images.')
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(
        ...
        # add dynamic_process_v3
        self.scale_threshold = scale_threshold
        if scale_threshold == 'v3':
            self.dynamic_preprocess_func = dynamic_preprocess_v3
        else:
            self.dynamic_preprocess_func = dynamic_preprocess
    ...
    def get_preprocess_function(self):
        ...
        if self.template_name == 'Hermes-2':
            ...
            preprocess_function = preprocess_internvl_v3
        else:
    ...
    def multi_modal_get_item(self, data_item):
        ...
        if self.dynamic_image_size:  # If dynamic image size is enabled, preprocess the image dynamically
            images = self.dynamic_preprocess_func(image, min_num=self.min_dynamic_patch, max_num=self.max_dynamic_patch, image_size=self.image_size, use_thumbnail=self.use_thumbnail)
    ...
    def multi_modal_multi_image_get_item(self, data_item):
        ...
        # downscale history images for GUI agent
        add_history = 'image_history' in data_item and len(data_item['image_history']) > 0
        if add_history:
            history_annos = data_item['image_history_annos']
            keep_last_step_size = False
            if data_item['width'][-2] == data_item['width'][-1]:
                keep_last_step_size = True

        images, num_tiles = [], []
        num_image = len(data_item['image'])
        for image_i, image_path in enumerate(data_item['image']):
            # Merge the image path
            image_path = self.get_image_path(image_path)
            # Load the image using tcs_loader if available, otherwise use PIL
            image = self.load_image(image_path)

            # draw the click point on history images
            if add_history:
                if image_i < len(history_annos):
                    if history_annos[image_i] is not None:
                        image = draw_click_to_image(image, *history_annos[image_i])
                    if keep_last_step_size:
                        if image_i < len(history_annos) - 1:
                            image = image.resize((self.image_size, self.image_size))
                    else:
                        image = image.resize((self.image_size, self.image_size))
            if image_i == 0:
                # for action reversal task
                if 'action_reversal_point' in data_item:
                    image = draw_click_to_image_pretrain(image, *data_item['action_reversal_point'])
                # for mistake-informed reattempt task
                if 'negative_bbox' in data_item:
                    for bbox in data_item['negative_bbox']:
                        image = draw_bbox_to_image(image, bbox)
                
            if self.dynamic_image_size:  # If dynamic image size is enabled, preprocess the image dynamically
                if self.scale_threshold == 'v3':
                    image = dynamic_preprocess_v3(image, min_num=self.min_dynamic_patch, max_num=self.max_dynamic_patch if len(data_item['image']) == 1 else self.max_multi_image_dynamic_patch,
...
def main():
    ...
    tcs_loader = TCSLoader('~/aoss.conf') if has_tcs_loader else None
    ...
    if training_args.do_train:
        checkpoint =  None #True # 20250215 change None
